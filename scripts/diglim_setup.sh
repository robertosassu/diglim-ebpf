...
function install()
{
	...
	question "Add exclude rule for events generated by IMA audit?" "y"
	if [ $answer = "y" ]; then
		echo "-a always,exclude -F msgtype=INTEGRITY_RULE" > \
			/etc/audit/rules.d/ima.rules
	fi
...
function uninstall()
{
	...
	question "Remove exclude rule for events generated by IMA audit?" "y"
	if [ $answer = "y" ]; then
		rm -f /etc/audit/rules.d/ima.rules
	fi
